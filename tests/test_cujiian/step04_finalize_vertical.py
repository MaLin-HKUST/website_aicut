...
def main() -> None:
    parser = argparse.ArgumentParser(description="Step 04: finalize(vertical_1080p) 联调测试")
    ...
    save_json(get_finalize_vertical_result_path(task_id), result)

    print("finalize(vertical_1080p) 完成")
    print(f"结果文件: {get_finalize_vertical_result_path(task_id)}")
    print(f"最终视频: {result['final_video_path']}")
    print(f"normalized video: {result.get('normalized_video_path')}")
    print(f"normalized process: {result.get('normalized_process_path')}")
