...
function removeRange(ranges: DeleteRange[], selection: Selection): DeleteRange[] {
  ...
  for (const range of ranges) {
    ...
    if (normalized.start > range.start) next.push({ start: range.start, end: normalized.start });
    if (normalized.end < range.end) next.push({ start: normalized.end, end: range.end });
  }
...
function selectionFullyDeleted(ranges: DeleteRange[], selection: Selection): boolean {
  ...
  for (let index = normalized.start; index < normalized.end; index += 1) {
    if (!rangeContainsIndex(ranges, index)) return false;
  }
  return true;
}
...
export const SmartCutScriptEditor = forwardRef<SmartCutScriptEditorHandle, SmartCutScriptEditorProps>(function SmartCutScriptEditor(
  { script, disabled = false, onScriptChange, onStateChange },
  ref,
) {
  ...
  function markDeleteSelection() {
    ...
    commit(addRange(ranges, normalizedSelection));
  }

  function restoreSelection() {
    if (!normalizedSelection) return;
    commit(removeRange(ranges, normalizedSelection));
  }
  ...
  useEffect(() => {
    onStateChange?.({ canMarkDelete, canRestore, canClear, selectedCount });
  ...
  useImperativeHandle(
    ...
    [canClear, canMarkDelete, canRestore],
  );
